...
| 分区 | 日期范围 | 用途 | 限制 |
|------|----------|------|------|
| research | 2015-01-01 → 2026-06-30 | 因子开发、IC计算、WF folds 训练/验证、extend 模拟考 (含毕业TEST①) | 可反复使用 |
| development | 2026-07-01 → 2026-12-31 | TEST② 一次性终审 (holdout, 预注册评估版本=v28) | **只跑一次** |
| test | 2026-07-01 → 2026-12-31 | TEST② (同 development 区间) | **只跑一次** |
| blind | 2027-01-01 → 2027-12-31 | 模拟盘跟踪 (daily_pipeline) | **永不回测** |

**方案C验证协议 (v5, 2026-08-29 更新)**:
...
- TEST① (2025-01-01 ~ 2026-06-30): 已于 2026-08-09 毕业并入 research,
  现作为 extend 模拟考区间使用 (冻结权重可重复跑, 不消耗 TEST 锁)
- TEST② (2026-07-01 ~ 2026-12-31): 稳定因子中位数 ICIR 冻结权重, **版本级只跑一次**
  (`.test_lock_v5`; 预注册评估版本=v28; 数据未覆盖区间末端时完备性守卫硬失败,
  即最早 2026-12-31 数据齐后开跑); 段末数据并入 research 后新版本自可正常复用该段
...
3. 组合验证 (research分区) → walk-forward 5折 + extend 模拟考 (冻结权重)
4. 最终测试 (TEST②分区, 本版本周期只跑一次) → 结果锁定, 作 scale-up go/no-go
...
## 第十一条：训练阶段迭代纪律（WF-7 级联协议，2026-08-29）

Walk-Forward 现为 7 折：核心 5 折（训练窗止于 2023，负责因子选型）+
级联折 6/7（训练至 2024/2025，验证 2025 全年与 2026H1，成绩链推进到研究段末尾）。

**证据分级（不可越级使用）:**
- **选型证据 = 核心 5 折**：稳定因子判定（≥3/5 hits）与中位数 ICIR 权重只统计
  核心折；fold_6/7 的 ICIR 不进选型（代码强制 `FOLD_CORE_N`）。
- **级联体检 = fold_6/7 + extend**：只回答"这套已定稿的模型在后续时段表现如何"，
  **禁止依据其数字反选参数/因子**。
- **终审 = TEST②/BLIND**：唯一的 go/no-go 裁判（见第一条）。

**迭代流程（每次版本变更必须遵守）:**
1. **预注册**：动选型或跑级联段之前，在 `experiment_tracker` 备注/报告 md 中写明
   假设（"因子X/参数Y 预期改善 Z 方向"）与接受标准，跑完不许改方向再跑。
2. **一次决策**：同一变更对同一验证段只判一次。**禁止变体锦标赛**——需要比较
   多个变体时，比较只发生在核心折训练窗（ICIR/显著性），验证段只判最终的"整套版本"。
3. **段毕业**：一段数据完成一次终审级评判后并入训练窗（如 fold_6 判完 2025，
   fold_7 训练窗即含 2025），验证链向前滚动，不复用旧考卷选型。
