...
# 当前生产实验 (2026-08-16 v26): top_k=20, 确定性 POV, 基准覆盖度过滤,
# 30 万资金; 含真实逐笔成交 + fill_times
PROD_RESULT = (Path(__file__).resolve().parents[3] / "data" / "ic_validation"
...
def _load_prod_extend() -> dict | None:
    """加载当前生产 EXTEND 模拟考数据: {positions_history, equity_curve, weights_evolution}。"""
    if not PROD_RESULT.exists():
        return None
    try:
        d = json.loads(PROD_RESULT.read_text(encoding="utf-8"))
        return d.get("results", {}).get("extend_val", {})
    except Exception:
        return None


def _reconstruct_prod_trades(ev: dict) -> list[dict]:
    """按回测真实逻辑重建成交记录 (EXTEND 2025-01~2026-06 兜底路径)。

    模拟规则 (与 run_walkforward_backtest 一致):
      - 初始资金 300,000, top_k=20, 整手 100 股
      - 成交价 = 调仓日 VWAP 近似 (仅 JSON 无真实 trades 时兜底)
    ...
    _cache = getattr(_reconstruct_prod_trades, "_vwap_cache", {})
        ...
        if sym not in _cache:
            ...
            if not (upath.exists() and apath.exists()):
                ...
                try:
                    ...
                    _cache[sym] = None
        _reconstruct_prod_trades._vwap_cache = _cache
        vf = _cache.get(sym)
...
def backtest_trades():
    """当前生产实验 (EXTEND 模拟考) 的调仓记录。

    优先使用回测 JSON 中记录的**真实逐笔成交** (v24c 起回测写入,
    含真实成交价/数量/佣金); 旧版 JSON 无 trades 字段时回退重建近似。
    """
    ev = _load_prod_extend()
    if not ev:
        return {"available": False, "count": 0, "trades": [],
                "note": "walkforward_results.json 不存在"}
    # 真实逐笔成交 vs 重建近似
    ...
    if real_trades:
        ...
        trades = _reconstruct_prod_trades(ev)
    ...
    return {
        "available": True,
        "version": "v26 (top_k=20, 确定性POV, 基准覆盖度过滤)",
